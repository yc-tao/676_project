...
**Files:**
- Create (if absent): `pyproject.toml`
...
Run:
```bash
ls pyproject.toml src tests 2>/dev/null; python -c "import torch; print(torch.__version__, torch.backends.mps.is_available())"
```
Expected: either files print or `ls` errors (fine). Torch must import and MPS must be available (`True`). If MPS is `False`, stop and ask the user — this plan assumes MPS.

- [ ] **Step 2: Write `pyproject.toml` if none exists**

Only create this file if the `ls` in Step 1 did not show `pyproject.toml`. Contents:
...
pip install -e .[dev]
```
Expected: `Successfully installed rq1_dlnm-0.1.0` (and deps, if missing). If pip complains about a missing backend, re-run with `pip install --upgrade pip setuptools wheel` first.

- [ ] **Step 4: Write `src/rq1_dlnm/__init__.py`**
...
- [ ] **Step 5: Write `tests/rq1_dlnm/__init__.py`**

Empty file — just create it.

- [ ] **Step 6: Write `tests/rq1_dlnm/conftest.py`**
...
- [ ] **Step 7: Verify test harness works**
...
```bash
git add pyproject.toml src/rq1_dlnm/__init__.py tests/rq1_dlnm/__init__.py tests/rq1_dlnm/conftest.py
git commit -m "chore: bootstrap rq1_dlnm package and test harness"
```
...
Run:
```bash
python -c "from rq1_dlnm.run_sweep import run; df = run(); print(df.head()); print('n pairs:', len(df))"
```
...
```bash
git add src/rq1_dlnm/run_sweep.py tests/rq1_dlnm/test_model.py results/summary.csv results/figures
...
Create `rq1_dlnm_demo.ipynb` with these cells. Use `jupyter nbconvert --to notebook --execute` or just write the JSON directly — the simplest way is:
...
If that heredoc doesn't parse in your shell, create the notebook manually in Jupyter with the three cells above.

- [ ] **Step 2: Execute the notebook to confirm it runs end-to-end**

Run:
```bash
jupyter nbconvert --to notebook --execute rq1_dlnm_demo.ipynb --output rq1_dlnm_demo.ipynb
```
...
pip install -e .[dev]
pytest -q
python -c "from rq1_dlnm.run_sweep import run; run()"
```
...
Run:
```bash
python scripts/sanity_temp_respiratory.py
```
